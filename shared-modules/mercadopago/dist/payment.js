...
exports.createUsagePayment = createUsagePayment;
exports.verifyWebhookSignature = verifyWebhookSignature;
exports.createPayment = createPayment;
const crypto_1 = require("crypto");
const client_1 = require("./client");
function notificationUrl(request) {
    ...
    if (baseApi)
        return `${baseApi}/api/pagos/webhook`;
    return undefined;
}
/**
 * Crea un pago por uso simple (pago único)
 */
async function createUsagePayment(config, usageConfig, externalReference) {
    const client = new client_1.MercadoPagoClient(config);
    const { pricePerRaffle, currency = 'ARS', description = 'Pago por sorteo', } = usageConfig;
...
function verifyWebhookSignature(xSignature, xRequestId, dataId, secret) {
    try {
        ...
        const calculated = (0, crypto_1.createHmac)('sha256', secret).update(manifest).digest('hex');
 ...
 */
async function createPayment(config, request) {
    const client = new client_1.MercadoPagoClient(config);
    const { title, description, price, currency = 'ARS', quantity = 1, externalReference, successUrl, failureUrl, pendingUrl, notificationUrl: notificationUrlReq, } = request;
    const notification = notificationUrl();
    const preferenceData = {
        items: [
            {
