...
### Base de Datos
- [x] Crear proyecto PostgreSQL gratis (Supabase o Neon)
- [x] Cambiar `provider = "postgresql"` en `api/prisma/schema.prisma`
- [x] Actualizar `DATABASE_URL` en `.env` con la URL de la nube
- [x] Generar migración inicial con `prisma migrate dev --name init`
- [x] Verificar que todos los modelos (Usuario, Sorteo, Participante, Certificado) funcionan en Postgres
- [x] Aplicar migraciones en producción (`prisma migrate deploy`)

### API (Deploy)
- [x] Subir el repo a GitHub (incluyendo shared-modules o empaquetarlas)
- [x] Crear servicio Node en Render/Railway conectado al repo
- [x] Configurar variables de entorno (DATABASE_URL, JWT_SECRET, MP_ACCESS_TOKEN)
- [x] Instalar Playwright en el entorno de build (`npx playwright install --with-deps chromium`)
- [x] Ajustar CORS en `api/src/index.ts` para el dominio de la web
- [x] Agregar rate limiting básico a los endpoints públicos
...
- [x] Fix (Log 42): instalación de browsers Playwright movida DESPUÉS del `npm install` — fabricaba versión no coincidente con el runtime y `|| npx playwright install chromium` silenciaba la falta del canal chrome; se agregó `xvfb` explícito, espera del socket X en el CMD y fallback del orquestador a Chromium headless cuando no hay DISPLAY. **Verificado en vivo**: Chrome real + Xvfb arrancan y corren en Render (estado "Live").
- [x] Evaluar Estrategia G en nube: instalar Chrome real (canal `chrome` de Playwright) + `xvfb` en el Dockerfile de Render funciona, pero **el plan free (512 MB) NO alcanza**: el contenedor muere por OOM durante el scroll anónimo ("Instance failed: came out of memory (used over 512MB)"). Fix `9af19c4`/`c3b29a5` fue necesario para arrancar, y se le agregaron flags de bajo consumo + heap Node 384 MB, pero no basta para scroll infinito.
- [ ] **Decisión pendiente (sesión 2026-08-07):** el MVP gratis no puede usar Chrome real headful en Render free. Opciones documentadas en Log 43: (a) Apify como primario en la nube cuando hay `APIFY_TOKEN` sin sesión — corre en servidores de Apify y no gasta la RAM local (candidato natural para "reducir el modelo gratis"); (b) limitar scroll/iteraciones en la nube; (c) plan de pago para mantener la Estrategia G al 99%. **Nada se elimina**: Estrategia G y x2f/gp quedan en standby para cuando se escale de pago, y se reutilizan con el cambio de plan.
...
- [x] Confirmar que los datos persisten (recargar, esperar, verificar en Supabase) — Log 40: `/api/sorteos/cuota` responde datos reales de la DB desde Render vía pooler
