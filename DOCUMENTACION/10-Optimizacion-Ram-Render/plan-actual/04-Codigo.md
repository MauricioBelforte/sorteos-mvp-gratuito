...
### N° 2 — Reciclado de página en `estrategiaScrollAnonimo`

- **IMPLEMENTADO** en `scroll-anon-completo.ts`: función `recargarPagina(motivo)` extraída del bloque de reinicio (reescribe `page.close()` + `newPage()` + `goto` si hay contexto anónimo, o `reload()` si no), y el bucle la llama por dos vías:
  - Reinicio por sin-progreso (comportamiento existente, ahora con la función).
  - **Reciclado periódico** cada `ITERACIONES_POR_RECICLO = 40` iteraciones (con tope `RECICLADOS_MAXIMOS = 8`), que resetea `sinProgreso`/`rebotado` (es avance, no no-progreso).

## Hallazgo en la prueba local (2026-08-07)

- **`--single-process` CRASHEA Chrome real** en Windows (el navegador se desconecta al navegar). **NO usar.** Se mantienen: `--no-zygote`, `--js-flags=--max-old-space-size=384`, `--expose-gc`, `--disable-software-rasterizer`, `--disable-features=TranslateUI,VizDisplayCompositor`. Esto cambió lo previsto en `plan-inicial/03-Diseno.md` (era documentar `--single-process`).
